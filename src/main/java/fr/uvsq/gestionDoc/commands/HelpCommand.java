...
public class HelpCommand implements Command 
{
	...
	public void execute() 
	{
		if (commande == null || commande.equals("add")) {
			System.out.println(ansi().a("\t").bold().a("add").reset().fgBrightYellow().a(" <chemin fichier>").reset().a(" : ajoute le fichier dont le chemin est <chemin fichier> à la base de données.\n"));
		}	
		...
		if (commande == null || commande.equals("show")) {
			System.out.println(ansi().a("\t").bold().a("show").reset().fgBrightYellow().a(" -extension=<ext> -auteur=<aut> -date=<dt> -type=<ty>").reset().a(" : affiche les fichiers contenus dans la base de données, avec possibilité d'affiner la recherche avec les options en utilisant la syntaxe ").fgBrightYellow().a("-critere=valeur").reset().a(".\n"));
		}	
